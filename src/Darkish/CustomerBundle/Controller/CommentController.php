...
class CommentController extends Controller
{
	...
	/**
	 * @Route("/customer/ajax/comment/search", defaults={"_format"="json"})
	 * @Method({"POST"})
	 */
	public function searchAction(Request $request) {
		...
		return new Response($this->get('jms_serializer')->serialize(array('comments'=>$comments), 'json', SerializationContext::create()->setGroups(array('Default', 'comment.details'))));
	}
	...
	public function setClaimAction(Comment $comment) {
	    try {
	        ...
	        $em->persist($comment);
	        $em->flush();
	        return new Response($this->get('jms_serializer')->serialize($comment, 'json', SerializationContext::create()->setGroups(array('Default', 'comment.details'))));
	    } catch(\Exception $e) {
	...
	public function replyCommentAction(Request $request, Comment $comment) {
	    ...
	    if ($form->isValid()) {
	    	...
	    	$em->persist($child);
	        if ($em->flush() !== false) {
	            return new Response($this->get('jms_serializer')->serialize($child, 'json', SerializationContext::create()->setGroups(array('Default', 'comment.details'))));
	        }
	    }
