...
//        return new Response($this->get('jms_serializer')->serialize($offer, 'json'));
    ...
    public function totalSearchOffersAction($keyword, $search_by, $sort_by) {
        $repo = $this->getDoctrine()->getRepository('DarkishCategoryBundle:Offer');
        $qb = $repo->createQueryBuilder('r');
        /* @var $qb QueryBuilder */
        ...
        switch($search_by) {
            case '1':
                ...
                break;
            case '2':
                $qb->where($qb->expr()->like('r.offerNumber', $qb->expr()->literal('%' . $keyword . '%')));
                break;
            case '3':
                $qb->orWhere($qb->expr()->like('r.title', $qb->expr()->literal('%' . $keyword . '%')));
                $qb->orWhere($qb->expr()->like('r.subTitle', $qb->expr()->literal('%' . $keyword . '%')));
        ...
        switch($sort_by) {
            case '4':
                $qb->orderBy('r.offerNumber', 'Asc');
                break;
            case '3':
                $qb->orderBy('r.offerNumber', 'Desc');
                break;
            case '2':
                $qb->orderBy('r.creationDate', 'Asc');
                break;
    ...
    public function getTotalOfferForCategoryAction($cid) {
        ...
        if($cid == -1) {
            ...
            $serialized = $this->get('jms_serializer')->
                serialize($newsList, 'json', SerializationContext::create()->setGroups(array('offer.list')));
        ...
        if($cid == 0) {
            ...
            $offerWithoutTree = $qb2->where($queryBuilder->expr()->notIn('rr.id',$offerWithTree->getDQL()))
                ->setFirstResult(0);
            $serialized = $this->get('jms_serializer')->
                serialize(count($offerWithoutTree->getQuery()->getResult()), 'json', SerializationContext::create()->setGroups(array('offer.list')));
